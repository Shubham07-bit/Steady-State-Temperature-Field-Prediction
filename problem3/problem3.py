...
class PhysicsInformedNN():
    ...
    def net_inside(self, x, y):
        ...
        Q_laser = (2 * self.A * self.P / (np.pi * self.r_b**2)) * torch.exp((-2) * (x**2 +(y - 100e-6)**2)/ self.r_b**2) * self.alpha * torch.exp(-self.alpha*y)
        # Q_laser = (2 * self.P / (np.pi * self.r_b**2)) * torch.exp(-2 * (x**2 + (y - 100e-6)**2/ self.r_b**2))
